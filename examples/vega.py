"""
This script demonstrates how to use the PyIndi library to control a telescope
and a CCD camera simulator. It connects to an INDI server, points the telescope
to the star Vega, takes exposures using the CCD simulator, and processes the
received image data (BLOBs).
"""
import PyIndi
import time
import sys
...
# Set the desired coordinates for the goto
...
# Send the updated coordinates property to the device to start the goto
...
while i < len(exposures):
    ...
    if i + 1 < len(exposures):
        ...
        indiclient.sendNewProperty(ccd_exposure)
    # Process the received BLOB data
